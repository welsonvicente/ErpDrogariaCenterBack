 ...
 * um lançamento, quem concedeu o atalho pro painel do gerente a um
 * funcionário...) — hoje isso só existia como log estruturado do servidor
 * (não consultável pelo gerente pela tela). `usuarioNome`/`usuarioEmail` são
...
export class RegistroAuditoria {
  ...
  /** Ex.: "despesa.editada", "despesa.excluida", "usuario.papel_promovido". */
  @Column({ length: 60 })
  acao!: string;
