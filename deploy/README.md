...
```bash
mkdir -p /var/www/apierp-drogariacenter
```

Crie `/var/www/apierp-drogariacenter/.env` (nunca vai pelo git) com os valores reais de produção — veja `backend/.env.example` para a lista completa de variáveis. Pontos importantes:
...
```bash
cp deploy/apierp-drogariacenter.service /etc/systemd/system/apierp-drogariacenter.service
systemctl daemon-reload
systemctl enable apierp-drogariacenter
```
